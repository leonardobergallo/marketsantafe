...
export function filterListings(filters: ListingFilters): Listing[] {
  ...
  // Usamos searchInString para búsquedas que ignoran acentos y mayúsculas
  if (filters.q) {
    filtered = filtered.filter(
      (listing) =>
        searchInString(listing.title, filters.q!) ||
        searchInString(listing.description, filters.q!)
    )
  }
